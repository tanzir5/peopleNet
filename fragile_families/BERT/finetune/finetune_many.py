...
def get_train_dict(main_train_df, background_encoded_df, task):
  train_df = copy.deepcopy(main_train_df[main_train_df[task].notna()])
  train_df.rename(columns={task: 'label'}, inplace=True)

  idxs = []
  input_ids = []
  labels = []
  for i in range(len(train_df)):
    current_idx = train_df.iloc[i][CHALLENGE_ID]
...
def get_train_eval_dataset(main_train_df, background_encoded_df, task, doing_mean_emb=False):
  train_dict = get_train_dict(main_train_df, background_encoded_df, task)
