...
def load_background(path):
  if TEST == "MEDIUM":
    df = pd.read_csv(path, skiprows=lambda x: x%20!=0)
    print("RAZOR")
...
def load_model(label):
  ...
  if MODEL_STRATEGY == WHOLE_MEAN_STRATEGY:
    ...
    path += label + "_residual_frozen_single/best/"
...
def eval_task(main_test_df, background_encoded_df, target_label, k_samples=10):
  ...
  for i in range(len(main_test_df)):
    ...
    current_diff_prediction = run_data(model, current_id, background_encoded_df, k_samples)
  ...
  R_df.append(results)  
  print(results, target_label, k_samples)
  print("-"*100)
max_seq_length = 512
...
if BINARY_PRETRAINED:
  final_df.to_csv('binary_whole_diff_k_results.csv', index=False)
else:
  final_df.to_csv('diff_k_results.csv', index=False)
