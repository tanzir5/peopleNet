...
def train_task(main_train_df, background_encoded_df, task, output_dir, model_path):
...
for i, chunk in tqdm(enumerate(chunk_df['chunk'].tolist())):
  ...
  if chunk_idx < 17:
    continue
  root_dir = str(chunk_idx) + "/"
  text_path = root_dir+'text.csv'
  if os.path.exists(text_path) is False:
    continue
  print("$"*100)
  print(i,"chunks done")
  chunk = ast.literal_eval(chunk)
  tokenizer = BertTokenizerFast.from_pretrained(root_dir+'tokenizer')
          
  background_df = pd.read_csv(root_dir+'text.csv')
  background_encoded_df = tokenize_background(background_df, tokenizer)
  model_path = root_dir+'final_models'
  for task in tasks:
    print("working for", task)
    train_df = pd.read_csv('../../Baseline/train/'+task+'_residual_train.csv')
    train_task(train_df, background_encoded_df, task, root_dir, model_path)
